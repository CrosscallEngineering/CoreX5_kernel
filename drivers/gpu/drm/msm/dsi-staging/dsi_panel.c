...
extern int aw99703_set_brightness_for_lcd_use(int brt_val);
#endif
#ifdef CONFIG_BACKLIGHT_KTZ8866
extern void ktz8866_brightness(int brightness);
#endif
...
static int dsi_panel_gpio_request(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(panel->vddio_1v8_en)) {
		rc = gpio_request(panel->vddio_1v8_en, "vddio_1v8_en");
		if (rc) {
			pr_err("request for vddio_1v8_en failed, rc=%d\n", rc);
			goto error_vddio_1v8_en;
		}
	}
	if (gpio_is_valid(panel->vdd_3v3_en)) {
		rc = gpio_request(panel->vdd_3v3_en, "vdd_3v3_en");
		if (rc) {
			pr_err("request for vdd_3v3_en failed, rc=%d\n", rc);
			goto error_vdd_3v3_en;
		}
	}

	if (gpio_is_valid(panel->vdd_2v_en)) {
		rc = gpio_request(panel->vdd_2v_en, "vdd_2v_en");
		if (rc) {
			pr_err("request for vdd_2v_en failed, rc=%d\n", rc);
			goto error_vdd_2v_en;
		}
	}
...
error_vdd_2v_en:
	if (gpio_is_valid(panel->vdd_3v3_en))
		gpio_free(panel->vdd_3v3_en);
error_vdd_3v3_en:
	if (gpio_is_valid(panel->vddio_1v8_en))
		gpio_free(panel->vddio_1v8_en);
error_vddio_1v8_en:
	if (gpio_is_valid(r_config->lcd_mode_sel_gpio))
...
static int dsi_panel_power_on(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(panel->vddio_1v8_en))
		gpio_direction_output((panel->vddio_1v8_en), 1);

	if (gpio_is_valid(panel->vdd_2v_en))
		gpio_direction_output((panel->vdd_2v_en), 0);
	
	if (gpio_is_valid(panel->vdd_3v3_en))
		gpio_direction_output((panel->vdd_3v3_en), 1);
...
error_disable_gpio:
	...
	if (gpio_is_valid(panel->vddio_1v8_en))
		gpio_direction_output((panel->vddio_1v8_en), 0);

	if (gpio_is_valid(panel->vdd_3v3_en))
		gpio_direction_output((panel->vdd_3v3_en), 0);
...
static int dsi_panel_power_off(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(panel->vdd_2v_en)){
		gpio_direction_output((panel->vdd_2v_en), 1);
	}
...
static int dsi_panel_parse_gpios(struct dsi_panel *panel)
{
	...
	panel->vddio_1v8_en = utils->get_named_gpio(utils->data,
						"qcom,platform-1v8en-gpio",
						0);
	if (!gpio_is_valid(panel->vddio_1v8_en))
		pr_debug("%s:%d vddio 1v8 en is not specified\n", __func__, __LINE__);

	panel->vdd_3v3_en = utils->get_named_gpio(utils->data,
						"qcom,platform-3v3en-gpio",
						0);
	if (!gpio_is_valid(panel->vdd_3v3_en))
		pr_debug("%s:%d vdd 3v3 en is not specified\n", __func__, __LINE__);

	panel->vdd_2v_en = utils->get_named_gpio(utils->data,
						"qcom,platform-2v-en-gpio",
						0);
	if (!gpio_is_valid(panel->vdd_2v_en))
		pr_debug("%s:%d vdd 2v en is not specified\n", __func__, __LINE__);
...
{
	int rc = 0;
///PSBP @{ added by liushulin for L820panel rotate 
	u8 tfupdate_rotate = 0x03;	
	struct mipi_dsi_device *dsi;
	dsi = &panel->mipi_device;
///PBSP @}
	if (!panel) {
	...
	mutex_lock(&panel->panel_lock);
///PSBP @{ added by liushulin for L820panel rotate 
	if((dev_bi.bootmode == BOOT_TFUPDATE_MODE) && (!strcmp(panel->name,"jd9365d guoxian wxga")))
	{
		mipi_dsi_dcs_write(dsi,0x36,&tfupdate_rotate,1);
		pr_err("rotate for recovery\n");
	}
///PBSP @}
