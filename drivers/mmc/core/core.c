				...
				bool cd_irq)
{
	...
	if(0 == mmc_schedule_delayed_work(&host->detect, delay))
		pr_err("%s: %s schedule_delayed_work fail\n", mmc_hostname(host), __func__);
}
...
int _mmc_detect_card_removed(struct mmc_host *host)
{
	...
	if (!host->card || mmc_card_removed(host->card)){
		pr_err("%s: %s card already removed\n", mmc_hostname(host), __func__);
		return 1;
	}
...
void mmc_rescan(struct work_struct *work)
{
	...
	if (host->bus_ops != NULL) {
		mmc_bus_put(host);
		pr_err("%s: %s bus_ops not NULL, card still present!!\n", mmc_hostname(host), __func__);
		goto out;
	}
