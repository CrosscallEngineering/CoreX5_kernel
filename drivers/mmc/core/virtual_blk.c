...
{
	struct virtualb *virtualb = rq->q->queuedata;
...
static blk_status_t virtual_handle_cmd(struct virtualb_cmd *cmd)
{
	...
	if (virtualb->dev->badblocks.shift != -1) {
		...
		if (!is_flush && badblocks_check(&virtualb->dev->badblocks, sector,
	...
	if (dev->memory_backed) {
		if (dev->queue_mode == VIRTUAL_Q_BIO) {
			if (bio_op(cmd->bio) == REQ_OP_FLUSH)
				err = virtual_handle_flush(virtualb);
			else
...
static void virtual_config_discard(struct virtualb *virtualb)
{
	...
	queue_flag_set_unlocked(QUEUE_FLAG_DISCARD, virtualb->q);
}
...
static int virtual_add_dev(struct virtualb_device *dev)
{
	...
	if (dev->queue_mode == VIRTUAL_Q_MQ) {
		...
		virtualb->q = blk_init_queue_node(virtual_request_fn, &virtualb->lock,
						dev->home_node);
		if (!virtualb->q) {
			rv = -ENOMEM;
			goto out_cleanup_queues;
	...
	if (dev->cache_size > 0) {
		...
		blk_queue_flush_queueable(virtualb->q, true);
	}
