...
int mmc_send_app_op_cond(struct mmc_host *host, u32 ocr, u32 *rocr)
{
	...
	for (i = 150; i; i--) {
		...
		if (err){
			pr_err("%s: %s CMD%d err=%d\n", mmc_hostname(host), __func__, cmd.opcode, err); 
			break;
		}
	...
	if (!i)
		pr_err("%s: %s card never left busy state, err=%d\n", mmc_hostname(host), __func__, err);
