...
void sdhci_send_command(struct sdhci_host *host, struct mmc_command *cmd)
{
	...
	/*ADD: cmd6 -110 timeout workaround*/
	if(MMC_SWITCH == cmd->opcode)
	{
		pr_debug("%s: caculated cmd6 timeout=0x%08x\r\n", __func__, sdhci_readb(host, SDHCI_TIMEOUT_CONTROL));
		sdhci_writeb(host, 0xE, SDHCI_TIMEOUT_CONTROL);//Set maximum timeout
	}
	/*ADD: end*/
