...
static camera_otp_valid_t hmct_otp_valid[4];
static int otp_syfs_created = 0;
#endif
...
#ifdef CONFIG_HMCT_CAMERA_DEBUG
static ssize_t hs_cam_otp_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int i, otp_num, len;
	int offset = 0;
	otp_num = sizeof(hmct_otp_valid)/sizeof(hmct_otp_valid[0]);

	for (i = 0; i < otp_num; i++) {
		pr_err("hmct_otp_valid[%d] %s\n", i, hmct_otp_valid[i].otp_state);
		if (strlen(hmct_otp_valid[i].otp_state) > 0) {
			len = snprintf(buf + offset, sizeof(hmct_otp_valid) - offset, "%s;", hmct_otp_valid[i].otp_state);
			offset += len;
		}
	}
...
static ssize_t hs_cam_otp_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
	...
	for (i = 0; i < otp_num; i++) {
		pr_err("hmct_otp_valid[%d] %s\n", i, hmct_otp_valid[i].otp_state);
		if (!strncmp(buf, hmct_otp_valid[i].otp_state, 6)
			|| !strlen(hmct_otp_valid[i].otp_state)) {
			break;
		}
	}
	if (i >= otp_num) {
		pr_err("hmct_otp_valid[%d] overflow.\n", i);
		return -EFAULT;
	}
	
	if (sscanf(buf, "%s",hmct_otp_valid[i].otp_state) != 1) {
		pr_err("eeprom_dbg_store: sscanf is wrong!\n");
		return -EINVAL;
	}
	
	return len;
}
static DEVICE_ATTR(hs_cam_otp, S_IWUSR | S_IWGRP | S_IRUGO,
		hs_cam_otp_show, hs_cam_otp_store);
#endif
static int msm_eeprom_platform_probe(struct platform_device *pdev)
{
	...
	if (e_ctrl->userspace_probe == 0) {
		...
		e_ctrl->is_supported = 1;
#ifdef CONFIG_HMCT_CAMERA_DEBUG
	if (otp_syfs_created == 0) {
		if (his_register_sysfs_attr(&dev_attr_hs_cam_otp.attr) < 0 ) {
			pr_err("Error creating hs_cam_otp!\n");
		}
			pr_err("%s create hs_cam_otp success!\n", __func__);
		otp_syfs_created = 1;
	}
#endif
