...
void StateMachineTypeC(struct Port *port)
{
    ...
    port->TCIdle = FALSE;
#if !defined(CONFIG_FUSB_PD_SNK_9VCHG)
...
#endif/*CONFIG_FUSB_PD_SNK_9VCHG*/
    {
        ...
        pr_info("FUSB bytes irq = %d inta=%2x, intb=%2x, s0=%2x, s1=%2x, int=%2x\n",
			platform_get_device_irq_state(port->PortID),
	...
	pr_info("FUSB302B : %s ConnState=%d\n", __func__, port->ConnState);
