...
void DP_RequestPartnerStatus(struct Port *port)
{
    ...
    /* saved mode position */
    svdmh.SVDM.ObjPos = 0x01;// port->DisplayPortData.DpModeEntered & 0x7;
...
FSC_BOOL DP_ProcessCommand(struct Port *port, FSC_U32* arr_in)
{
    ...
    if (port->DisplayPortData.DpEnabled == FALSE)
		return TRUE;
    svdmh_in.object = arr_in[0];
    is_ack = (svdmh_in.SVDM.CommandType == RESPONDER_ACK) ? TRUE : FALSE;

    pr_info("FUSB302B : %s object=0x%x, is_ack=%d\n", __func__, arr_in[0], is_ack);
