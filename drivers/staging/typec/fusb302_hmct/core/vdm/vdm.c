                          ...
                          FSC_U32 length_in)
{
	...
	{
	    pr_info("FUSB302B : %s object[%d]=0x%x\n", __func__, i, arr_in[i]);
	}
	...
	/*To solve Not bypass attention to DP, when SVdmver not equal to SVDM Version in ATTENTION package*/
        if ((DPM_SVdmVer(port, sop) != vdmh_in.SVDM.Version) &&(vdmh_in.SVDM.Command !=ATTENTION))
        {
	    pr_err("FUSB302B : %s Invalid structured VDM version,SvdmVer=0x%x, Version 0x%x,command 0x%x\n", __func__,DPM_SVdmVer(port, sop),vdmh_in.SVDM.Version,vdmh_in.SVDM.Command);
            /* Invalid structured VDM version */
            return 1;
        }

        if ( vdmh_in.SVDM.SVID == SVID1_SOP) {
            pr_debug("FUSB %s, arr_in[0]=%x\n", __func__, arr_in[0]);
            pr_debug("FUSB %s, arr_in[1]=%x\n", __func__, arr_in[1]);
            port->vdm_expectingresponse = FALSE;
            SetPEState(port, port->PolicyIsSource == TRUE ? peSourceReady : peSinkReady);
            notify_observers(DP_EVENT, port->I2cAddr, arr_in);

            switch (vdmh_in.SVDM.Command)
            {
            case DISCOVER_IDENTITY:
                port->AutoVdmState = AUTO_VDM_DISCOVER_ID_PP;
                break;
            case DISCOVER_SVIDS:
    ...
    {
	pr_err("FUSB302B : %s  Not support Unstructured  DPM_SpecRev 0x%x \n", __func__,DPM_SpecRev(port, sop));    
