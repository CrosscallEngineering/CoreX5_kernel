...
int sitronix_get_rawdata_rowcol(struct sitronix_ts_data *data, u8 *tx, u8 *rx)
{
	int ret;
	ret = sitronix_get_xy_chs(data);
	if (ret < 0)
		return ret;
	*rx = data->ts_dev_info.rx_chs;
	*tx = data->ts_dev_info.tx_chs;
	return 0;
}
int st_enter_glove_mode(struct sitronix_ts_data *ts)
{
	char buffer[2];
	int ret = 0;
	STX_INFO("%s enter.glove mode is %d\n",__func__,ts->glove_mode);
	if (ts->glove_mode)
	{
		STX_INFO("%s write enter glove mode.\n",__func__);
		...
		ts->glove_mode = true;
		STX_INFO("%s write enter glove mode done!\n",__func__);
	}
	STX_INFO("%s exit.\n",__func__);
	return ret;
}
...
int st_leave_glove_mode(struct sitronix_ts_data *ts)
{
	char buffer[2];
	int ret = 0;
	STX_INFO("%s enter.ts->glove_mode is %d\n",__func__,ts->glove_mode);
	if(!ts->glove_mode)
	{
		STX_INFO("%s write exit glove mode.\n",__func__);
		ret = stx_i2c_read(ts->client, buffer, 1, DEVICE_CONTROL_REG);
		if (ret < 0)
		{
			STX_ERROR(" i2c communcate error in getting DeviceControl : 0x%x", ret);
		...
		ts->glove_mode = false;
		STX_INFO("%s write exit glove mode done!\n",__func__);
	}
	STX_INFO("%s exit.\n",__func__);
	return ret;
}
