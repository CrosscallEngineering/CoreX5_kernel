...
#define KEY_GESTURE_DOUBLE_TAP 509
...
int sitronix_swk_func(struct input_dev *dev)
{
	...
	if (sitronix_swk_flag_get() == 1)
	{
		...
		case GESTURE_DOUBLECLICK:
			gesture = KEY_GESTURE_DOUBLE_TAP;	//KEY_POWER; //KEY_GESTURE_U;
			break;
		case GESTURE_O:
...
int st_gesture_init(void)
{
	...
	input_set_capability(input_dev, EV_KEY, KEY_GESTURE_DOUBLE_TAP);	//KEY_POWER
	...
	__set_bit(KEY_GESTURE_DOUBLE_TAP, input_dev->keybit);	//KEY_POWER
...
void sitronix_gesture_func(struct input_dev *input_dev, int id)
{
	if (id == G_PALM)
	{
		...
		input_report_key(input_dev, KEY_GESTURE_DOUBLE_TAP, 1); // KEY_POWER 1);
		input_sync(input_dev);
		input_report_key(input_dev, KEY_GESTURE_DOUBLE_TAP, 0); //KEY_POWER
		input_sync(input_dev);
	}
}
#endif
