...
struct sitronix_ts_data
{
	...
	u16 gesture_state;
#endif /* CONFIG_TOUCHSCREEN_SITRONIX_TS_GESTURE */
};
...
extern int sitronix_get_rawdata_rowcol(struct sitronix_ts_data *data, u8 *tx, u8 *rx);
extern int sitronix_ts_get_rawdata(struct device *dev,  char *buf);
extern int sitronix_ts_get_short_test(struct device *dev,  char *buf);
//extern int sitronix_ts_get_open_test(struct device *dev,  char *buf);
extern int sitronix_get_fs_fw_version(struct device *dev, char *buf);
extern int sitronix_proc_fw_update(struct device *dev, bool force);
extern int sitronix_check_fw_update_need(struct device *dev);
