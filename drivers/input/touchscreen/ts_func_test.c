...
*
* If TP realize this interface---->int (*ts_async_suspend_for_lcd_use)(void), 
* LCD will make TP enter suspend. It can help INCELL IC avoid gesture mode
* or  power problem.
*/
...
*	   This function is provided for LCD ASYNC DISPLAY.
*
* If TP realize this interface---->int (*ts_async_resume_for_lcd_use)(void), 
* LCD will make TP enter resume. It can help INCELL IC avoid gesture mode
* or  power problem.
*/
...
*
* If TP realize this interface---->int (*ts_reset_for_lcd_use)(void), 
* LCD will use it to make TP reset first, then the sequence can satisfy the IC's need.
*/
...
*	   This function is provided for INCELL IC sequence.
*
* If TP realize this interface---->int (*ts_suspend_need_lcd_reset_high)(void), 
* LCD will use it to make TP reset keep high when suspend, then the sequence can satisfy the IC's need.
*/
...
*
* If TP realize this interface---->int (*ts_suspend_need_lcd_reset_high)(void), 
* LCD will use it to keep LCD power and reset high when suspend, then the gesture
* can work.
*/
...
static ssize_t diff_show(struct kobject  *obj, struct kobj_attribute *attr, char *buf)
{
	struct ts_func_test_device *test_device;
	int ret = 0;
	int path_num;
...
static ssize_t selftest_show(struct kobject  *obj, struct kobj_attribute *attr, char *buf)
{
	struct ts_func_test_device *test_device;
	int ret = 0;
	int path_num;
...
static ssize_t fw_fs_settinginfo_show(struct kobject  *obj, struct kobj_attribute *attr, char *buf)
{
	struct ts_func_test_device *test_device;
	int ret = 0;
	int path_num;
...
static ssize_t fw_set_erase_flash_test(struct kobject  *obj, struct kobj_attribute *attr, const char *buf, size_t size)
{
	struct ts_func_test_device *test_device;
	int path_num;
...
static ssize_t factory_info_show(struct kobject  *obj, struct kobj_attribute *attr, char *buf)
{
	struct ts_func_test_device *test_device;
	int ret = 0;
	int path_num;
...
static ssize_t factory_cfg_info_show(struct kobject  *obj, struct kobj_attribute *attr, char *buf)
{
	struct ts_func_test_device *test_device;
	int ret = 0;
	int path_num;
...
static ssize_t gesture_fingers_show(struct kobject  *obj, struct kobj_attribute *attr, char *buf)
{
	struct ts_func_test_device *test_device;
	int ret = 0;
	int path_num;
...
static ssize_t irq_awake_show(struct kobject  *obj, struct kobj_attribute *attr, char *buf)
{
	struct ts_func_test_device *test_device;
	bool enable = false;
	int path_num;
...
static ssize_t irq_awake_store(struct kobject  *obj, struct kobj_attribute *attr, const char *buf, size_t size)
{
	struct ts_func_test_device *test_device;
	int path_num;
	...
	if (test_device && test_device->set_tp_irq_awake_switch) {
		if (strncmp(buf, "0", 1)) {
			test_device->set_tp_irq_awake_switch(test_device->dev, true);
		} else {
			test_device->set_tp_irq_awake_switch(test_device->dev, false);
		}
		return size;
	}
	return -EINVAL;
...
static ssize_t check_gpio_state_show(struct kobject  *obj, struct kobj_attribute *attr, char *buf)
{
	struct ts_func_test_device *test_device;
	bool state = false;
	int path_num;
...
static ssize_t tp_match_hw_show(struct kobject  *obj, struct kobj_attribute *attr, char *buf)
{
	struct ts_func_test_device *test_device;
	bool enable = false;
	int path_num;
...
static ssize_t tp_work_mode_show(struct kobject  *obj, struct kobj_attribute *attr, char *buf)
{
	struct ts_func_test_device *test_device;
	int path_num;
...
static ssize_t tp_work_mode_store(struct kobject  *obj, struct kobj_attribute *attr, const char *buf, size_t size)
{
	struct ts_func_test_device *test_device;
	int path_num;
	int ret = 0;
