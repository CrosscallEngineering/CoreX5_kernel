...
static int fts_pram_write_init(struct fts_upgrade *upg)
{
    ...
    FTS_INFO("check whether tp is in romboot or not ");
    ...
    FTS_INFO("after write pramboot, confirm run in pramboot");
...
int fts_fwupg_enter_into_boot(void)
{
    ...
    if (upg->func->pramboot_supported) {
        ...
        FTS_INFO("pram not supported, confirm in bootloader");
        /* bootloader */
...
int fts_fwupg_ecc_cal(u32 saddr, u32 len)
{
    ...
    for (i = 0; i < packet_num; i++) {
        ...
        FTS_INFO("ecc calc startaddr:0x%04x, len:%d", addr, packet_len);
...
static bool fts_lic_need_upgrade(struct fts_upgrade *upg)
{
    ...
    FTS_INFO("vid in tp:0x%04x, host:0x%04x", vid_in_tp, vid_in_host);
    ...
    FTS_INFO("lcd initial code version in tp:%x, host:%x",
    ...
    if (0xA5 == initcode_ver_in_tp) {
        ...
        FTS_INFO("lcd init code in tp is invalid, need upgrade init code");
        return true;
...
static int ft5526_get_fw_filename(struct fts_ts_data *data, u8 pannel_id, char *buf, int size)
{
	char tmp[16] = {0};
	const char *str = "NULL";
...
int ft5526_get_fs_fw_version(struct device *dev, char *buf)
{
	...
	FTS_INFO("%s enter.");
	ft5526_get_fw_filename(data, data->module_id, fw_file_name, sizeof(fw_file_name));
	FTS_INFO( "fw_file_name=%s\n", fw_file_name);

    ret = request_firmware(&fw, fw_file_name, dev);
    if (0 == ret) {
        FTS_INFO("firmware(%s) request successfully", fw_file_name);
		/*FT5526-003  .fwveroff = 0x0014,*/
		fs_fw_version = fw->data[20];
    } else {
        FTS_INFO("firmware(%s) request fail,ret=%d", fw_file_name, ret);
    }
    if (fw != NULL) {
        release_firmware(fw);
        fw = NULL;
	...
	FTS_INFO("0x%02X \n",  fs_fw_version);
	return snprintf(buf, 300, "0x%02X\n", fs_fw_version);
}
...
/*
	...
	ft5526_get_fw_filename(upg->ts_data, upg->ts_data->module_id, fwname, sizeof(fwname));
...
int ft5526_check_fw_update_need(struct device *dev)
{
	struct fts_ts_data *data = dev_get_drvdata(dev);
	const struct firmware *fw = NULL;
	char fw_file_name[128] = {0};
	...
	if(fs_fw_version > data->fw_ver[0])
		need_update = true;
...
int ft5526_proc_fw_update(struct device *dev, bool force)
{
	int ret = 0;
	struct fts_upgrade *upg = fwupgrade;
	...
	fts_irq_enable();
	upg->ts_data->fw_loading = 0;
	ft5526_ts_register_productinfo(upg->ts_data);
	return 0;
}
