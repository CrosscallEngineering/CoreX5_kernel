...
*
*/
 #define pr_fmt(fmt) "TP-focaltech-ctl: " fmt
...
static int fts_rw_iic_drv_myread(struct i2c_client *client, u8 *buf, int length)
{
	...
	if (ret < 0)
		printk(KERN_ERR "ft5x06 %s:IIC Read failed\n", __func__);
	...
	u8 *buf, int length)
{
	...
	if (ret < 0)
		printk(KERN_ERR "ft5x06 %s:IIC Write failed\n", __func__);
...
static void fts_rw_iic_drv_setup_cdev(struct fts_rw_i2c_dev *dev, int index)
{
	...
	if (err)
		printk(KERN_ERR "ft5x06 Error %d adding LED%d", err, index);
}
...
static int fts_rw_iic_drv_myinitdev(struct i2c_client *client)
{
	...
	if (err < 0) {
		printk(KERN_ERR "ft5x06 %s:ft_rw_iic_drv failed  error code=%d---\n", __func__, err);
		return err;
	}
	...
	if (!fts_rw_i2c_dev_tt) {
		...
		printk(KERN_ERR "ft5x06 %s:ft_rw_iic_drv failed\n", __func__);
		return err;
	}
	...
	if (IS_ERR(fts_class)) {
		printk(KERN_ERR "ft5x06 %s:failed in creating class.\n", __func__);
		return -ENODEV;
	}
	/*create device node*/
...
{
	printk(KERN_INFO "ft5x06 [FTS]----ft_rw_iic_drv init ---\n");
