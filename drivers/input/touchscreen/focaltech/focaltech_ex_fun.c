...
*
*/
 #define pr_fmt(fmt) "TP-focaltech-ex-func: " fmt
/*
	...
	const char __user *buff, size_t count, loff_t *ppos)
{
	...
	if(G_Client == NULL){
		printk(KERN_ERR "ft5x06 %s G_Client is NULL",__func__);
		return -ENODEV;
	}
	if (copy_from_user(&writebuf, buff, buflen)) {
		printk(KERN_ERR "ft5x06 %s:copy from user error\n", __func__);
		return -EFAULT;
	}
	...
	case PROC_READ_REGISTER:
		...
		if (ret < 0) {
			printk(KERN_ERR "ft5x06 %s:write iic error\n", __func__);
			return ret;
		}
		break;
	...
	char __user *buff, size_t count, loff_t *ppos)
{
	...
	case PROC_READ_REGISTER:
		...
		if (ret < 0) {
			printk(KERN_ERR "ft5x06 %s:read iic error\n", __func__);
			kfree(buf);
			return ret;
		}
		num_read_chars = 1;
	...
	if (copy_to_user(buff, buf, num_read_chars)) {
		printk(KERN_ERR "ft5x06 %s:copy to user error\n", __func__);
		kfree(buf);
		return -EFAULT;
	}
...
int fts_create_apk_debug_channel(struct i2c_client *client)
{
	...
	if (NULL == fts_proc_entry) {
		printk(KERN_ERR "ft5x06 Couldn't create proc entry!\n");
		return -ENOMEM;
	}
