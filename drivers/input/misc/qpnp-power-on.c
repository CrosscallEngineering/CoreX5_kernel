...
#define FS_SYNC_INTERVAL_MS_BEGIN	6000
...
static int qpnp_pon_input_dispatch(struct qpnp_pon *pon, u32 pon_type)
{
 ...
 && key_status)
	{
		schedule_delayed_work(&pon->fsync_timer, msecs_to_jiffies(FS_SYNC_INTERVAL_MS_BEGIN));
	}
				...
				u32 s1_timer, u32 s2_timer, u32 enable)
{
	...
	pr_info("%s: pon_type %d, s2_type %d, enable %d\n", __func__,
			pon_type, s2_type, enable);
	if (NULL == pon) {
...
{
	pr_info("%s: set resin debug to %d.\n", __func__, debug);

	if (NULL == sys_reset_dev) {
		WARN(1, "%s: sys_reset_dev is NULL\n", __func__);
		return;
	}

	if (debug) {
		qpnp_config_reset_for_hs(PON_RESIN,
			PON_POWER_OFF_WARM_RESET, 10256, 2000, 1);
