...
static long fpsensor_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
...
#ifdef CONFIG_SAVE_AWAKEN_EVENT
        register_irqnum_and_event(g_fpsensor->irq, AWAKEN_EVENT_FP);
#endif /* CONFIG_SAVE_AWAKEN_EVENT */
    ...
    case FPSENSOR_IOC_ENABLE_REPORT_BLANKON:
        ...
        break;
	case FPSENSOR_IOC_SET_VEN_INF:
		{
		char ictype[16];
		char fp_message[32];
		if (copy_from_user(ictype, (void __user *)arg, sizeof(ictype))) {
				fpsensor_debug(ERR_LOG, "Failed to copy ic type from user to kernel\n");
				retval = -EFAULT;
			}
		printk("fp_id(..)%s .\n",ictype);
		snprintf(fp_message,sizeof(fp_message),"chipone %s",ictype);
		productinfo_register(PRODUCTINFO_FINGERPRINT_ID,fp_message, NULL);
		printk("%s: ioctl product info send\n", __func__);
		}
		break;
    default:
...
//		return 0;
	...
	printk("[info] %s go to the fpsensor_fb_notifier_callback value = %d\n",
			__func__, (int)event);

/*    if (evdata && evdata->data && fpsensor_dev && event == MSM_DRM_EARLY_EVENT_BLANK) {
            transition = *(int *)evdata->data;
			switch(transition){
            case MSM_DRM_BLANK_POWERDOWN:
                fpsensor_debug(ERR_LOG,"%s: lcd off notify\n", __func__);
                sprintf(screen_status, "SCREEN_STATUS=%s", "OFF");
                fpsensor_dev->fb_status = 0;
			break;
            case MSM_DRM_BLANK_UNBLANK:
                fpsensor_debug(ERR_LOG,"%s: lcd on notify\n", __func__);
...
//    msm_drm_register_client(&fpsensor_dev->notifier);
#endif
