...
#define AW2023_VDD_MIN_UV					1800000
#define AW2023_VDD_MAX_UV					1800000
...
const unsigned char aw2023_bat2_reg_access[AW2023_REG_MAX] = {
...
static int aw2023_vdd_control(struct aw2023_led *led, bool on)
{
	int rc;

	if (led->vdd_always_on) {
		dev_info(&led->client->dev, "%s: vdd is always on\n", __func__);
		return 0;
	}
...
static int aw2023_power_on(struct aw2023_led *led, bool on)
{
	...
	rc = aw2023_vdd_control(led, on);
	if (rc) {
		dev_err(&led->client->dev, "vdd enable failed rc=%d\n", rc);
		return rc;
	}
...
fail_enable_reg:
	rc = aw2023_vdd_control(led, false);

	return rc;

fail_disable_reg:
	rc = aw2023_vdd_control(led, true);

	return rc;
}

static int aw2023_vdd_regulator_init(struct aw2023_led *led)
{
	int rc = 0;
...
static int aw2023_power_init(struct aw2023_led *led, bool on)
{
	...
	if (on) {
		rc = aw2023_vdd_regulator_init(led);
		if (rc) {
			dev_err(&led->client->dev, "vdd init failed rc=%d\n", rc);
			return rc;
		}
...
static void aw2023_led_blink_set(struct aw2023_led *led, unsigned long blinking)
{
	...
	pr_err("aw2023: set %s blink to %d\n", led->cdev.name, blinking);
				...
				enum led_brightness brightness)
{
	...
	pr_err("aw2023: set %s brightness to %d\n",
				...
				struct device_attribute *attr, char *buf)
{
	...
	for(i=0; i<AW2023_REG_MAX; i++) {
		if(!(aw2023_bat2_reg_access[i]&REG_RD_ACCESS))
		continue;
...
static ssize_t aw2023_led_cur_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t len)
{
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
    ...
    sscanf(buf, "%d", &led_cur);
    led->pdata->led_current = led_cur;

    return len;
}

static ssize_t aw2023_led_cur_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	struct aw2023_led *led =
...
static DEVICE_ATTR(led_cur, 0664, aw2023_led_cur_show, aw2023_led_cur_store);
...
static int aw2023_check_chipid(struct aw2023_led *led)
{
	...
	if (led->vdd_always_on) {
		// if vcc is always on ,we need to reset chip
				...
				const struct i2c_device_id *id)
{
	...
	led_array->vdd_always_on = of_property_read_bool(node,
				"aw2023,vdd-always-on");
...
static void aw2023_shutdown(struct i2c_client *client)
{
	struct aw2023_led *led_array = i2c_get_clientdata(client);

	// if vcc is always on ,we need to reset chip
	if (led_array && led_array->vdd_always_on)
		aw2023_write(led_array, AW2023_REG_RESET, AW2023_RESET_MASK);
}
...
static struct of_device_id aw2023_match_table[] = {
	{ .compatible = "awinic,bat2_aw2023_led",},
	{ },
};
...
static int __init aw2023_bat2led_init(void)
{
	return i2c_add_driver(&aw2023_led_driver);
}
module_init(aw2023_bat2led_init);

static void __exit aw2023_bat2led_exit(void)
{
	i2c_del_driver(&aw2023_led_driver);
}
module_exit(aw2023_bat2led_exit);
