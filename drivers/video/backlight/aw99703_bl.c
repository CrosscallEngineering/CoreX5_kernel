...
static int aw99703_gpio_init(struct aw99703_data *drvdata)
{
	...
	if (gpio_is_valid(drvdata->hwen_gpio)) {
		...
		if (ret < 0) {
			pr_err("failed to request hwen_gpio\n");
			return -1;
		}
		ret = gpio_direction_output(drvdata->hwen_gpio, 1);
		pr_info(" request hwen_gpio init\n");
		if (ret < 0) {
			pr_err("failed to set hwen_gpio output");
			gpio_free(drvdata->hwen_gpio);
			return ret;
		}
		pr_info("hwen_gpio is valid!\n");
...
int  aw99703_set_brightness(struct aw99703_data *drvdata, int brt_val)
{
	...
	if(last_brightness == brt_val){
		pr_info("%s Set Same Level:%d,Return!\n",__func__,brt_val);
		return 0;
	}
			...
			const struct i2c_device_id *id)
{
	...
	aw99703_set_brightness(drvdata, 614);//2047*30%
