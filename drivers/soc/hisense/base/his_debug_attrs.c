...
#define HISENSE_DEBUG_ATTR(attr_name, attr_offset) \
	...
	{ \
		...
		his_read_file(partition, attr_offset, &flag, sizeof(flag)); \
		\
...
static void hs_store_attr_value(int offset, int value)
{
	...
	his_write_file(partition, offset, &flag, sizeof(flag));
}
...
HISENSE_DEBUG_ATTR(hs_disable_wp, FLAG_HS_DISABLE_WP_OFFSET);
HISENSE_DEBUG_ATTR(audio_debug, FLAG_HS_AUDIO_DEBUG_OFFSET);
HISENSE_DEBUG_ATTR(debug_fs_switch, FLAG_DEBUG_FS_SWITCH_OFFSET);
...
static struct attribute *his_debug_ctrl_attrs[] = {
	...
	&dev_attr_hs_disable_wp.attr,
	&dev_attr_audio_debug.attr,
	&dev_attr_debug_fs_switch.attr,
	NULL,
};
