...
{
	struct timespec chr_start_ts;
	char time_str[32] = {0};

	memset(&chr_time, 0, sizeof(struct charger_time));

	get_monotonic_boottime(&chr_start_ts);
...
{
	struct timespec chr_stop_ts;
	char time_str[32] = {0};
	long chr_diff_time = 0;

	get_monotonic_boottime(&chr_stop_ts);
...
{
    int quo = 0, rem = 0;
    struct timespec chr_recoder_ts;
    ...
    if(!rem && (chr_time.chr_recorder[quo].status == false)) {
        get_monotonic_boottime(&chr_recoder_ts);
...
static ssize_t charge_time_recorder_show(struct device *dev,
...
static DEVICE_ATTR(charge_time_recorder, S_IRUGO, charge_time_recorder_show, NULL);

static int __init charger_flow_dbg_init(void)
{
	int ret;

	ret = his_register_sysfs_attr(&dev_attr_charge_time_recorder.attr);
	if (ret < 0) {
