...
/// BSP: add wlan_ftm interface to statistics packets received @{
static ssize_t wlan_ftm_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
	char str_total_pkt[10], str_good_pkt[10], str_crc_pkt[10];
	if (sscanf(buf, "totalPackets %s goodPackets %s crcPackets %s", str_total_pkt, str_good_pkt, str_crc_pkt) != 3) {
		pr_err("debug_store: sscanf is wrong!\n");
		return -EINVAL;
	}
	snprintf(wlan_ftm, sizeof(wlan_ftm), "totalPackets %s goodPackets %s crcPackets %s",
		str_total_pkt, str_good_pkt, str_crc_pkt);

	return strlen(wlan_ftm);
}

static ssize_t wlan_ftm_show(struct device *dev,
               struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%s", wlan_ftm);
 }
static DEVICE_ATTR(wlan_ftm, S_IWUSR | S_IWGRP | S_IRUGO, wlan_ftm_show, wlan_ftm_store);
/// @}
...
/// BSP: add wlan_ftm interface to statistics packets received @{
	his_register_sysfs_attr(&dev_attr_wlan_ftm.attr);
/// @}
