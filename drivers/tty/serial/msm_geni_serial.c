		...
		if (drv->cons) {
			...
			if (!line) {
				if(!get_debug_flag_bit(SERIAL_ENABLE_BIT)) {
					pr_info("Disable DEBUG UART %s%d(dev %s)  register\n",
						drv->dev_name, line, dev_name(&pdev->dev));
					return -ENODEV;
				} else {
					/* WAR FOR USER BUILD UART STOP ISSUE AFTER SUSPEND*/
					pr_info("Enable console during suspend\n");
