...
void reset_level_debug_set(void)
{
	bool enable = false;

	enable = get_debug_flag_bit(DEBUG_ENABLE_BIT);
	pr_info("%s: debug_flag is %d\n", __func__, enable);
	/* Enable debug, restart whole system, or only subsystem */
	if (enable)
		set_subsys_restart_level(RESET_SOC);
	else
		set_subsys_restart_level(RESET_SUBSYS_COUPLED);

	hs_set_pm_pon_resin(enable);
	/*2019-1-22,huangyaling add,
	solve resin+kpr warmreset cannot boot issue,
	keep dload_mode be set*/
	set_dload_mode(true);
}

int hs_disable_sdi(void)
{
	pr_info("%s now\n", __func__);
	scm_disable_sdi();
	return 0;
...
static void msm_restart_prepare(const char *cmd)
{
	...
	if (force_warm_reboot || need_warm_reset) {
		...
		smb5_configure_afp_enable(false);
#endif /*CONFIG_HISENSE_BATTERY_THERM_NTC_10K*/
	...
	if (cmd != NULL) {
		if (!strncmp(cmd, "bootloader", 10)) {
...
#ifdef CONFIG_HISENSE_BOOT_FLOW
			...
			qpnp_pon_set_restart_reason(
				PON_RESTART_REASON_HS_INITCALL_LOG);
#endif /* CONFIG_HISENSE_BOOT_FLOW */
		} else {
#ifdef CONFIG_HISENSE_BOOT_FLOW
			qpnp_pon_set_restart_reason(
