...
#define cw_bat2_printk(fmt, arg...)        \
	({                                    \
	 if(CWFG_ENABLE_LOG){              \
	 printk("FG_CW2015 BAT2 : %s : " fmt, __FUNCTION__ ,##arg);  \
	 }else{}                           \
	 })     //need check by Chaman


#define CWFG_NAME "cw2015_bat2_gauge"
...
int g_cw2015_bat2_capacity = 0;
int g_cw2015_bat2_vol = 0;
struct cw_battery *g_cw_bat2 = NULL;
...
int cw_update_config_info_bat2(struct cw_battery *cw_bat)
{
	int ret;
	...
	cw_bat2_printk("\n");
	cw_bat2_printk("[FGADC] test config_info = 0x%x\n",config_info[0]);
	printk(KERN_INFO "%s\n", __func__);
	...
	msleep(100);
	cw_bat2_printk("cw2015 update config success!\n");
...
int g_cw_get_bat2_voltage(void)
{
	int vol;
	if(!g_cw_bat2){
		pr_err( "Could not get cw2015_bat2\n");
		return 0;
	}
	vol = cw_get_voltage(g_cw_bat2);
	return vol;
}
EXPORT_SYMBOL(g_cw_get_bat2_voltage);
int g_cw_reset_bat2_fuelgauge(void)
{
	int vol =0;
	if(!g_cw_bat2){
		pr_err( "Could not get cw2015_bat2\n");
	...
	g_cw_bat2->isDataInit = false;
	//battery is out , you can send new battery capacity vol here what you want set
	//for example
	g_cw_bat2->capacity = 0;//100;
	g_cw_bat2->voltage = 3700;//4200;
	g_cw_bat2->change = 1;
	cancel_delayed_work_sync(&g_cw_bat2->battery_delay_work);
	cw_quick_soft_reset(g_cw_bat2);
	queue_delayed_work(g_cw_bat2->cwfg_workqueue, &g_cw_bat2->battery_delay_work, msecs_to_jiffies(50));
	return vol;
}
EXPORT_SYMBOL(g_cw_reset_bat2_fuelgauge);
...
static int get_charge_state(void)
{
	...
	if(!chrg_usb_psy){
		cw_bat2_printk("Usb online didn't find\n");
	}
	if(!chrg_dc_psy){
		cw_bat2_printk("Ac online didn't find\n");
	}
	cw_bat2_printk("ac_online = %d    usb_online = %d\n", ac_online, usb_online);
...
static int cw_init_data(struct cw_battery *cw_bat)
{
	...
	cw_bat2_printk("[%d]: real_SOC = %d, digit_SOC = %d\n", __LINE__, real_SOC, digit_SOC);
...
static int cw_quick_soft_reset(struct cw_battery *cw_bat)
{
	...
	if (ret < 0) {
		...
		cw_bat2_printk("Failed Write REG_MODE POR 0x%x!\n",reg_val);
		ret = cw_write(cw_bat->client, REG_MODE, &reg_val);
		if (ret < 0)
			return ret;
	}
	msleep(10);
	cw_bat2_printk("Success Write REG_MODE POR 0x%x!\n",reg_val);
	return ret;
}
...
static int cw_init(struct cw_battery *cw_bat)
{
	...
	if (!(reg_val & CONFIG_UPDATE_FLG)) {
		cw_bat2_printk("update config flg is true, need update config\n");
		ret = cw_update_config_info_bat2(cw_bat);
		if (ret < 0) {
		...
		if (i != SIZE_BATINFO) {
			cw_bat2_printk("config didn't match, need update config\n");
			ret = cw_update_config_info_bat2(cw_bat);
			if (ret < 0){
				return ret;
			}
	...
	if (i >= 30 ){
		reg_val = MODE_SLEEP;
		ret = cw_write(cw_bat->client, REG_MODE, &reg_val);
		cw_bat2_printk("cw2015 input unvalid power error, cw2015 join sleep mode\n");
		return -1;
	} 

	cw_bat2_printk("cw2015 init success!\n");	
	return 0;
}
...
static int cw_get_capacity(struct cw_battery *cw_bat)
{
	...
	real_SOC = reg_val[0];
	digit_SOC = reg_val[1];
	printk(KERN_INFO "CW2015_bat2[%d]: real_SOC = %d, digit_SOC = %d\n", __LINE__, real_SOC, digit_SOC);

	/*case 1 : avoid IC error, read SOC > 100*/
	if ((real_SOC < 0) || (real_SOC > 100)) {
		cw_bat2_printk("Error:  real_SOC = %d\n", real_SOC);
		reset_loop++;			
	...
	cw_bat2_printk("CW2015[%d]: ui_100 = %d, UI_SOC = %d, remainder = %d\n", __LINE__, ui_100, UI_SOC, remainder);
	/*case 3 : aviod swing*/
	if(UI_SOC >= 100){
...
int g_cw_get_bat2_capacity(void)
{
	int capacity= 0;
#ifdef CONFIG_HISENSE_SGM_DOUBLE_BATTERIES
	/*In factory mode not care battery exist or not*/
	if(dev_bi.bootmode != BOOT_FACTORY_MODE){
		if(!(sgm_check_battery2_exist())){
			/*battery1 not exist*/
			capacity = -1;
			pr_buf_info( "bat2 not exist\n");
			return capacity;
		}
	}
#endif /*CONFIG_HISENSE_SGM_DOUBLE_BATTERIES*/
	if(!g_cw_bat2){
		pr_buf_err( "Could not get cw2015_bat2\n");
		return capacity;
	}

	//capacity = cw_get_capacity(g_cw_bat2);
	capacity = g_cw_bat2->capacity;
	pr_buf_info( "bat2 cap %d\n",capacity);
	return capacity;
}

EXPORT_SYMBOL(g_cw_get_bat2_capacity);
static void cw_update_capacity(struct cw_battery *cw_bat)
{
	...
	if ((cw_capacity >= 0) && (cw_capacity <= 100) && (cw_bat->capacity != cw_capacity)) {				
		if (cw_capacity > cw_bat->capacity) {
			...
			cw_bat2_printk( "charger_mode %d, status %d, capacity %d, cw_capacity %d\n",cw_bat->charger_mode,cw_bat->status,cw_bat->capacity,cw_capacity);
		}else{
			cw_bat->capacity = cw_capacity;
		}
		cw_bat->change = 1;
...
static void cw_bat_work(struct work_struct *work)
{
	...
	cw_bat = container_of(delay_work, struct cw_battery, battery_delay_work);
       if(!cw_bat->isDataInit){
               cw_bat2_printk("Try Data Init again....\n");
               cw_bat->capacity = 0;//100;
               g_cw2015_bat2_capacity = cw_bat->capacity;
               ret = cw_init(cw_bat);
		while ((i++ < 3) && (ret != 0)) {
			msleep(200);
			ret = cw_init(cw_bat);
		}
                cw_bat2_printk("cw2015 init retry %d times\n", i);
		if(i >=3)
			goto next_cw2_work;
               ret = cw_init_data(cw_bat);
               if (ret) {
                       cw_bat2_printk("cw2015 init data fail,retry later!\n");
                       goto next_cw2_work;
               }
	...
	/*Add for battery swap end*/
	cw_bat2_printk("charger_mod = %d\n", cw_bat->charger_mode);
	cw_bat2_printk("status = %d\n", cw_bat->status);
	cw_bat2_printk("capacity = %d\n", cw_bat->capacity);
	cw_bat2_printk("voltage = %d\n", cw_bat->voltage);
	...
	g_cw2015_bat2_capacity = cw_bat->capacity;
	g_cw2015_bat2_vol = cw_bat->voltage;
next_cw2_work:
...
static int cw2015_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (!cw_bat) {
		cw_bat2_printk("cw_bat create fail!\n");
		return -ENOMEM;
	}
	...
	if(!ret)
		cw_bat->isDataInit = true;
	cw_bat2_printk("cw2015 driver probe success and cw_init_data %s!\n", ((cw_bat->isDataInit) ?" Success":"Fail"));
	return 0;
}
...
static struct of_device_id cw2015_match_table[] = {
	{ .compatible = "cellwise,bat2_cw2015", },
	{ },
};
...
static int __init cw215_bat2_init(void)
{
	//struct i2c_client *client;
...
static void __exit cw215_bat2_exit(void)
{
	i2c_del_driver(&cw2015_driver);
}

module_init(cw215_bat2_init);
module_exit(cw215_bat2_exit);
