...
#ifdef CONFIG_HISENSE_BATTERY_THERM_NTC_10K
static struct smb5 *global_smb5_chip = NULL;
#endif /*CONFIG_HISENSE_BATTERY_THERM_NTC_10K*/

#ifdef CONFIG_HISENSE_SGM41512_CHARGER
extern bool sgm_mag_charger_ok(void);
extern bool sgm_typec_charger_ok(void);
struct smb_charger *global_smb_charger = NULL;
#endif /*CONFIG_HISENSE_SGM41512_CHARGER*/

#ifdef CONFIG_HISENSE_SGM_DOUBLE_BATTERIES
extern bool sgm_2btt_mag_charger_ok(void);
extern bool sgm_2batt_typec_charger_ok(void);
extern bool sgm_use_battery1_ok(void);
...
#if defined(CONFIG_HISENSE_SGM_DOUBLE_USB) ||defined(CONFIG_HISENSE_SY6974_CHARGER)
struct smb_charger *global_smb_charger = NULL;
#endif
...
static int __debug_mask = PR_PARALLEL | PR_INTERRUPT | PR_OTG | PR_MISC | PR_REGISTER;
module_param_named(
...
#ifdef CONFIG_HISENSE_SGM41512_CHARGER
		if (sgm_mag_charger_ok() && (!sgm_typec_charger_ok())) {
			if (!chg->sgm_chg_psy) {
				chg->sgm_chg_psy= power_supply_get_by_name("sgm_charger");
				if (!chg->sgm_chg_psy) {
					pr_buf_err("Couldn't get sgm_chg_psy power_supply\n");
					val->intval = POWER_SUPPLY_TYPE_USB_DCP;
					break;
				}
			}

			power_supply_get_property(chg->sgm_chg_psy, POWER_SUPPLY_PROP_TYPE, val);
		}
#endif /*CONFIG_HISENSE_SGM41512_CHARGER*/

#ifdef CONFIG_HISENSE_SGM_DOUBLE_BATTERIES
		if (sgm_2btt_mag_charger_ok() && (!sgm_2batt_typec_charger_ok())) {
			/*Force return Float charge type*/
...
static int smb5_init_usb_psy(struct smb5 *chip)
{
	...
	if (IS_ERR(chg->usb_psy)) {
		pr_buf_err("Couldn't register USB power supply\n");
...
static int smb5_init_usb_port_psy(struct smb5 *chip)
{
	...
	if (IS_ERR(chg->usb_port_psy)) {
		pr_buf_err("Couldn't register USB pc_port power supply\n");
...
static int smb5_init_usb_main_psy(struct smb5 *chip)
{
	...
	if (IS_ERR(chg->usb_main_psy)) {
		pr_buf_err("Couldn't register USB main power supply\n");
...
#ifdef CONFIG_HISENSE_CW2015_FG
	POWER_SUPPLY_PROP_CAPACITY_BAT1, /* for battery 1 */
	POWER_SUPPLY_PROP_CAPACITY_BAT2, /* for battery 2 */
#endif /*CONFIG_HISENSE_CW2015_FG*/
#ifdef CONFIG_HISENSE_SGM_DOUBLE_BATTERIES
	POWER_SUPPLY_PROP_STATUS_BAT1, /* for battery 1 */
	POWER_SUPPLY_PROP_STATUS_BAT2, /* for battery 2 */
#endif /*CONFIG_HISENSE_SGM_DOUBLE_BATTERIES*/
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		break;
#ifdef CONFIG_HISENSE_CW2015_FG
	case POWER_SUPPLY_PROP_CAPACITY_BAT1:/* for battery 1 */
		val->intval = g_cw_get_bat1_capacity();
		break;
	case POWER_SUPPLY_PROP_CAPACITY_BAT2: /* for battery 2 */
		val->intval = g_cw_get_bat2_capacity();
		break;
#endif /*CONFIG_HISENSE_CW2015_FG*/
#ifdef CONFIG_HISENSE_SGM_DOUBLE_BATTERIES
	case POWER_SUPPLY_PROP_STATUS_BAT1: /* for battery 1 */
		rc = smblib_get_prop_batt_status_bat1(chg,val);
		break;
	case POWER_SUPPLY_PROP_STATUS_BAT2: /* for battery 2 */
		rc = smblib_get_prop_batt_status_bat2(chg,val);
		break;
#endif /*CONFIG_HISENSE_SGM_DOUBLE_BATTERIES*/
...
#ifdef CONFIG_HISENSE_CHARGE_FG_FUNCTION
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		pr_buf_err("%s set charging_enabled=%d\n", __func__, val->intval);
...
static int smb5_init_batt_psy(struct smb5 *chip)
{
	...
	if (IS_ERR(chg->batt_psy)) {
		pr_buf_err("Couldn't register battery power supply\n");
...
#ifdef CONFIG_FUSB_30X
	/* FUSB302 will register dual role class, so return it*/
	return rc;
#endif /*CONFIG_FUSB_30X*/
...
static int smb5_init_vbus_regulator(struct smb5 *chip)
{
	...
	if (IS_ERR(chg->vbus_vreg->rdev)) {
		...
		if (rc != -EPROBE_DEFER)
			pr_buf_err("Couldn't register VBUS regulator rc=%d\n", rc);
	}
...
#ifdef CONFIG_HISENSE_BATTERY_THERM_NTC_10K
void smb5_configure_afp_enable(bool enable)
{
	int rc = 0;
	u8 val = 0, mask = 0;

	if (enable) {
		mask = JEITA_EN_AFP_HOT_BIT | JEITA_EN_AFP_COLD_BIT;
		val = JEITA_EN_AFP_HOT_BIT | JEITA_EN_AFP_COLD_BIT;
	} else {
		mask = JEITA_EN_AFP_HOT_BIT | JEITA_EN_AFP_COLD_BIT;
		val = 0;
	}
	...
	if (global_smb5_chip) {
		rc = smblib_masked_write(&global_smb5_chip->chg, JEITA_EN_CFG_REG,
			mask, val);
		if (rc < 0)
			pr_err("%s: %s AFP failed rc=%d\n", __func__, enable ? "enable" : "disable", rc);
	}

	pr_err("%s: %s AFP done. rc=%d.\n", __func__, enable ? "enable" : "disable", rc);

}
EXPORT_SYMBOL_GPL(smb5_configure_afp_enable);
#endif /* CONFIG_HISENSE_BATTERY_THERM_NTC_10K */
...
#ifdef CONFIG_HISENSE_BATTERY_THERM_NTC_10K
	//enable afp function.
	smb5_configure_afp_enable(true);
#endif /* CONFIG_HISENSE_BATTERY_THERM_NTC_10K */
...
#ifdef CONFIG_HISENSE_CHARGE_FG_FUNCTION
       /*Set default aicl threshold to 4100mv*/
       chg->default_aicl_5v_threshold_mv = 4100;
#endif /* CONFIG_HISENSE_CHARGE_FG_FUNCTION */
...
#else /*CONFIG_FUSB_30X*/
		rc = smblib_masked_write(chg, PMI632_TYPEC_MICRO_USB_MODE_REG,
				MICRO_USB_MODE_ONLY_BIT, MICRO_USB_MODE_ONLY_BIT);
		if (rc < 0) {
			dev_err(chg->dev, "Couldn't configure MICRO_USB_MODE rc=%d\n", rc);
			return rc;
		}

		rc = smblib_masked_write(chg, TYPEC_U_USB_CFG_REG, EN_MICRO_USB_MODE_BIT,
				EN_MICRO_USB_MODE_BIT);
		if (rc < 0) {
			dev_err(chg->dev, "Couldn't force to Micro-B mode rc=%d\n", rc);
			return rc;
		}
		type = MICRO_USB_MODE_ONLY_BIT;
#endif /*CONFIG_FUSB_30X*/
	}
...
int smb5_usbin_plugin_irq_enable(bool enable)
{
	if (enable){
		enable_irq_wake(smb5_irqs[USBIN_PLUGIN_IRQ].irq);
	}else{
		disable_irq_wake(smb5_irqs[USBIN_PLUGIN_IRQ].irq);
	}
	return 0;
}
EXPORT_SYMBOL_GPL(smb5_usbin_plugin_irq_enable);
#endif /*CONFIG_HISENSE_SGM_DOUBLE_BATTERIES*/
...
#ifdef CONFIG_HISENSE_SGM41512_CHARGER
	global_smb_charger = chg;
#endif /*CONFIG_HISENSE_SGM41512_CHARGER*/

#if defined(CONFIG_HISENSE_SGM_DOUBLE_USB) ||defined(CONFIG_HISENSE_SY6974_CHARGER)
	global_smb_charger = chg;
#endif
	pr_buf_info("QPNP SMB5 probed successfully\n");
