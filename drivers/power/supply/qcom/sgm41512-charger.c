...
//extern function from sgm_double_charger.c
extern bool sgm_mag_charger_ok(void);
extern bool sgm_typec_port_online(void);
extern int sgm_usb_gpio_switch(bool enable);
...
static int sgm41512_update_charger_status(struct sgm41512_charger_info *info)
{
	...
	if (global_smb_charger && !sgm_typec_charger_ok()) {
				  ...
				  u32 limit_cur)
{
	...
	pr_buf_info("USBIN limit_current = %d\n", limit_cur);
...
static int sgm41512_charger_start_charge(struct sgm41512_charger_info *info)
{
	...
	if (!info->chg_en_gpio) {
		...
		pr_buf_info("set chg_en_gpio 0\n");
	}

	info->charging = true;
out:
	//mutex_unlock(&info->lock);
	//pr_buf_info("start charge, ret = %d.\n", ret);
...
static void sgm41512_charger_stop_charge(struct sgm41512_charger_info *info)
{
	...
	if(!info->chg_en_gpio) {
		...
		pr_buf_info("###STOP charger### cmd to set disable charge\n");
	} else {
		gpio_direction_output(info->chg_en_gpio, 1);
		pr_buf_info("###STOP charger### set chg_en_gpio to high\n");
	}
...
static int sgm41512_set_prop_temp_control(struct sgm41512_charger_info *info)
{
	...
	pr_buf_err("batt_health is %d, mag_chg %d, gd_vbus %d\n",
		info->batt_health_status, sgm_mag_charger_ok(), info->good_vbus_detected);

	//check charging status.
	if (!sgm_mag_charger_ok() || !info->good_vbus_detected)
		return 0;
				      ...
				      int val)
{
	...
	if (val == 3 && info->charging) {
		sgm41512_charger_stop_charge(info);
		pr_buf_info("sgm41512_charger_stop_charge\n");
	}
	else if (val == 2)
	{
		...
		if (ret)
		    ...
		    pr_buf_info("sgm41512_charger start charge\n");
		}
	}
...
static void sgm41512_charger_work(struct work_struct *data)
{
	...
	if (present && info->good_vbus_detected) {
		...
		if ((info->charger_type == POWER_SUPPLY_TYPE_USB
			|| info->charger_type == POWER_SUPPLY_TYPE_USB_CDP)
			&& (info->usb_mode == false) && !sgm_typec_port_online()) {
			pr_buf_info("USB detected, set to USB mode.\n");
			sgm_usb_gpio_switch(true);
...
static void sgm41512_determine_initial_status(struct sgm41512_charger_info *info)
{
	sgm41512_update_charger_status(info);
	sgm41512_update_fault_status(info);
		...
		const struct i2c_device_id *id)
{
	...
	sgm41512_determine_initial_status(info);

	enable_irq_wake(info->int_irq);

	pr_buf_info("sgm41512_charger_probe success\n");
	return 0;
...
static int sgm41512_charger_remove(struct i2c_client *client)
{
	...
	int ret;
	pr_buf_err("sgm41512 otg mode %d\n",IS_ERR_OR_NULL(info));
