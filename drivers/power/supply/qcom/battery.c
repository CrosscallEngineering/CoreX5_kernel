...
#define pr_fmt(fmt) "--PM-BATT: %s: " fmt, __func__
...
static int get_adapter_icl_based_ilim(struct pl_data *chip)
{
	...
	pl_dbg(chip,PR_PARALLEL,"adapter_icl=%d, chip->chg_param->hvdcp3_max_icl_ua %d\n",adapter_icl,
		chip->chg_param->hvdcp3_max_icl_ua);
	/*
...
static void split_settled(struct pl_data *chip)
{
	...
	if (rc < 0) {
        ...
        pr_err("%s main_ua=%d,slave_ua=%d,total_settled_ua=%d\n",
               __func__,main_ua,slave_ua,total_settled_ua);
	/*
...
static void handle_settled_icl_change(struct pl_data *chip)
{
	...
	total_current_ua = get_effective_result_locked(chip->usb_icl_votable);
	pr_err("%s:total_current_ua = %d\n", __func__,total_current_ua);
	...
	main_limited = pval.intval;
        pr_err("%s:tmain_limited = %d,main_settled_ua=%d,chip->pl_settled_ua=%d\n",
                __func__,main_limited,main_settled_ua,chip->pl_settled_ua);
