...
#define pr_fmt(fmt)	"--PM-QG-K: %s: " fmt, __func__
...
static int qg_debug_mask = QG_DEBUG_IRQ | QG_DEBUG_STATUS | QG_DEBUG_PON;
...
static int qpnp_qg_probe(struct platform_device *pdev)
{
	...
	qg_get_battery_capacity(chip, &soc);
	pr_buf_info("QG initialized! battery_profile=%s SOC=%d QG_subtype=%d\n",
