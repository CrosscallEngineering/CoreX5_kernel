 ...
 */
#define pr_fmt(fmt) "--PM-PMIC: %s: " fmt, __func__
...
extern bool jeita_bat_ov;
#define USB_RECHECK_COUNT		5
#endif /*CONFIG_HISENSE_CHARGE_FG_FUNCTION*/

#ifdef CONFIG_FUSB_30X
extern bool get_fusb_probe_status(void);
extern void fusb_cancel_apsd_check_work(void);
extern void fusb_notify_apsd_complete(void);
extern void fusb_reinit_apsd_complete(void);
#endif /*CONFIG_FUSB_30X*/
...
#if defined(CONFIG_SMB1355_SLAVE_CHARGER) && defined(CONFIG_HISENSE_CHARGE_FG_FUNCTION)
extern bool smb1355_ftm_disabled;
#endif /*CONFIG_SMB1355_SLAVE_CHARGER && CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
				  ...
				  const union power_supply_propval *val)
{
	...
	rc = smblib_masked_write(chg, CHGR_SAFETY_TIMER_ENABLE_CFG_REG,
		FAST_CHARGE_SAFETY_TIMER_EN, !!(val->intval) ? FAST_CHARGE_SAFETY_TIMER_EN : 0);
	pr_buf_err("@@@@%s intval=%d rc=%d\n", __func__, val->intval, rc);
	return rc;
}
#endif /*CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
...
#endif /*#ifdef CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
...
#ifdef CONFIG_HISENSE_CHARGE_FG_FUNCTION
	/*hmct start, add for disable charging when usb otg mode*/
	prop.intval = 1;
	pr_buf_info("set charging_disabled\n");
	rc = smblib_set_prop_input_suspend(chg, &prop);
	if (rc < 0)
		smblib_dbg(chg, PR_OTG, "Couldn't set charging disabled rc = %d\n");
#endif /*#ifdef CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
...
int smblib_vbus_regulator_disable(struct regulator_dev *rdev)
{
    ...
    /*hmct start, add for enable charging when usb device mode*/
	pr_buf_err("set charging_enabled\n");
    rc = smblib_set_prop_input_suspend(chg, &prop);
    if (rc < 0)
        smblib_dbg(chg, PR_OTG, "Couldn't set charging enabled rc = %d\n");
#endif /*#ifdef CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
...
{
	u8 value = 0;
	smblib_read(chg, CHGR_SAFETY_TIMER_ENABLE_CFG_REG, &value);
	val->intval = !!(value & FAST_CHARGE_SAFETY_TIMER_EN);
	pr_buf_err("@@@@%s intval=0x%x, val->intval= %d\n", __func__, value, val->intval);
	return 0;
}
#endif /*CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
...
#if defined(CONFIG_HISENSE_SGM_DOUBLE_BATTERIES) && defined(CONFIG_HISENSE_CW2015_FG)
	if((sgm_check_battery1_exist()) &&(sgm_check_battery2_exist())){
		val->intval= ((g_cw_get_bat1_capacity()) + (g_cw_get_bat2_capacity()))/2;
		rc = 0;
	}else if(sgm_check_battery1_exist()){
		val->intval= (g_cw_get_bat1_capacity());
		rc = 0;
	}else if(sgm_check_battery2_exist()){
		val->intval = (g_cw_get_bat2_capacity());
		rc = 0;
	}else{
		smblib_err(chg, "Unknow batt ID status, battery 1 or 2 are all not exist.\n");
	}
	return rc;
#else
				...
				union power_supply_propval *val)
{
	...
	if (sgm_mag_charger_ok()) {
		if (!chg->sgm_chg_psy) {
			chg->sgm_chg_psy= power_supply_get_by_name("sgm_charger");
			if (!chg->sgm_chg_psy) {
				smblib_err(chg, "Couldn't get sgm_chg_psy power_supply\n");
				val->intval = POWER_SUPPLY_STATUS_CHARGING;
				return 0;
			}
		}

		power_supply_get_property(chg->sgm_chg_psy, POWER_SUPPLY_PROP_STATUS, &pval);
...
#if defined(CONFIG_HISENSE_SGM_DOUBLE_BATTERIES) && defined(CONFIG_HISENSE_CW2015_FG)
	if (sgm_2btt_mag_charger_ok()) {
		if(sgm_use_battery1_ok()){
			if (!chg->sy_chg1_psy) {
				chg->sy_chg1_psy= power_supply_get_by_name(SY6974X_CHG_BAT1_PSY_NAME);
				if (!chg->sy_chg1_psy) {
					smblib_err(chg, "Couldn't get sy6974_bat1_charger power_supply\n");
					val->intval = POWER_SUPPLY_STATUS_CHARGING;
					return 0;
				}
			}
			rc = power_supply_get_property(chg->sy_chg1_psy, POWER_SUPPLY_PROP_STATUS, &pval);
			if (rc < 0) {
				smblib_err(chg, "Couldn't get sy_chg1_psy prop status property rc=%d\n",rc);
				return rc;
			}
			val->intval = pval.intval;
		}else{
			if (!chg->sy_chg2_psy) {
				chg->sy_chg2_psy= power_supply_get_by_name(SY6974X_CHG_BAT2_PSY_NAME);
				if (!chg->sy_chg2_psy) {
					smblib_err(chg, "Couldn't get sy6974_bat2_charger power_supply\n");
					val->intval = POWER_SUPPLY_STATUS_CHARGING;
					return 0;
				}
			}
			rc = power_supply_get_property(chg->sy_chg2_psy, POWER_SUPPLY_PROP_STATUS, &pval);
			if (rc < 0) {
				smblib_err(chg, "Couldn't get sy_chg2_psy prop status property rc=%d\n",rc);
				return rc;
			}
			val->intval = pval.intval;
		}
		...
		if (rc < 0) {
			smblib_err(chg, "Couldn't get batt capacity property rc=%d\n",
			rc);
			return rc;
		}
		if((val->intval == POWER_SUPPLY_STATUS_CHARGING) && (pval.intval == 100)) {
		...
		if((val->intval == POWER_SUPPLY_STATUS_FULL) && (pval.intval != 100)) {
			val->intval = POWER_SUPPLY_STATUS_CHARGING;
			return 0;
		}
		return 0;
...
#ifdef CONFIG_HISENSE_SGM_DOUBLE_BATTERIES
int smblib_get_prop_batt_status_bat1(struct smb_charger *chg,
				union power_supply_propval *val)
{
	union power_supply_propval pval = {0, };
	bool usb_online, dc_online;
	u8 stat;
	int rc, suspend = 0;
	bool bat1_exist = false;
	int bat1_cap = 0;
	
	unsigned long bat_chg=sgm_2bat_get_chg_flag();
	if (sgm_2btt_mag_charger_ok()) {
			if(bat_chg&0x05){
				/*Battery 1 charging*/
				if (!chg->sy_chg1_psy) {
				...
				power_supply_get_property(chg->sy_chg1_psy, POWER_SUPPLY_PROP_STATUS, &pval);
				val->intval = pval.intval;
			}else{
	...
	if(bat_chg&0x05){
		...
		bat1_exist = sgm_check_battery1_exist();
		if(bat1_exist)
			pval.intval = bat1_cap;
	}else{
		val->intval = POWER_SUPPLY_STATUS_DISCHARGING;
	}	
	if((val->intval == POWER_SUPPLY_STATUS_CHARGING) && (pval.intval == 100)) {
...
int smblib_get_prop_batt_status_bat2(struct smb_charger *chg,
				union power_supply_propval *val)
{
	union power_supply_propval pval = {0, };
	bool usb_online, dc_online;
	...
	if(sgm_2btt_mag_charger_ok()){
			if(bat_chg&0x0A){
				...
				power_supply_get_property(chg->sy_chg2_psy, POWER_SUPPLY_PROP_STATUS, &pval);
				val->intval = pval.intval;
			}else{
				val->intval = POWER_SUPPLY_STATUS_DISCHARGING;
			}
	...
	if(bat_chg&0x0a){
		...
		bat2_exist = sgm_check_battery2_exist();
		if(bat2_exist)
			pval.intval = bat2_cap;
	}else{
		val->intval = POWER_SUPPLY_STATUS_DISCHARGING;
	}	
				...
				const union power_supply_propval *val)
{
	...
	if (val->intval > chg->thermal_levels)
		return -EINVAL;
#ifdef CONFIG_HISENSE_CHARGE_FG_FUNCTION
	if (BOOT_FACTORY_MODE == dev_bi.bootmode) {
		pr_buf_err("%s:thermal-enine not work in ftm mode , prev=%d, now=%d\n",
			__func__, chg->system_temp_level, val->intval);
		return -EINVAL;
	}
	pr_buf_info("%s:thermal-enine set fcc %d , prev=%d, now=%d\n",
			__func__, chg->thermal_mitigation[val->intval], chg->system_temp_level, val->intval);
#endif /*CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
...
#ifdef CONFIG_HISENSE_CHARGE_FG_FUNCTION
    /*Add for Jeita 4.1v tigger suspend input,  result in rebooting repeatedly in power-off-charging*/
    if(!val->intval && jeita_bat_ov) {
	    pr_info("%s, USB not online && jeita battery ov\n", __func__);
	    if((stat & USE_USBIN_BIT) && (chg->real_charger_type!=POWER_SUPPLY_TYPE_UNKNOWN)) {
            pr_info("%s, USB is plugged actually, force usb online\n", __func__);
            val->intval = 1;
	    }
    }
#endif /*CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
					...
					int usb_current)
{
	...
	pr_buf_err("%s apsd_result->pst =%, real_charger_type %d, usb_current %d mA\n", __func__, apsd_result->pst ,chg->real_charger_type, usb_current);
#endif /*CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_FLOAT) {
		if (usb_current == -ETIMEDOUT) {
			...
			if (chg->connector_type ==
				...
				pr_buf_err("%s set float current %d mA,charge type %d\n",
						__func__,chg->float_icl_ua,chg->real_charger_type);
#else/**CONFIG_FUSB_30X*/
				rc = vote(chg->usb_icl_votable,
					SW_ICL_MAX_VOTER, true, chg->dcp_icl_ua);
#endif/**CONFIG_FUSB_30X*/
#else/*CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
				rc = vote(chg->usb_icl_votable,
				...
				const union power_supply_propval *val)
{
	...
	if (chg->pd_active) {
#ifdef	CONFIG_FUSB_PD_SNK_9VCHG
	/*To resolve PD adapter  run into 9V, USBIN_OV INT*/
	rc = smblib_masked_write(chg, USB_USBIN_ADAPTER_ALLOW_REG, USB_USBIN_ADAPTER_ALLOW_MASK,
			USBIN_ADAPTER_ALLOW_5V_TO_9V);
	if (rc < 0)
		dev_err(chg->dev, "Couldn't set 0x%x to ADAPTER_ALLOW_5V_TO_9V, rc=%d\n",
				USB_USBIN_ADAPTER_ALLOW_REG, rc);

#endif	/*CONFIG_FUSB_PD_SNK_9VCHG*/
		vote(chg->limited_irq_disable_votable, CHARGER_TYPE_VOTER,
				false, 0);
	...
	if (rc < 0)
		dev_err(chg->dev, "Couldn't set 0x%x to ADAPTER_ALLOW_5V_OR_9V, rc=%d\n",
				USB_USBIN_ADAPTER_ALLOW_REG, rc);
	...
	pr_buf_info("@@@@%s pd_adapter_allowance_override=%d\n", __func__, chg->pd_active);
...
#ifdef CONFIG_HISENSE_CHARGE_FG_FUNCTION
			current_ua = chg->dcp_icl_ua;
                       smblib_dbg(chg, PR_MISC, "apsd_result: %d, set icl to %d\n",
                               apsd_result->bit,  chg->dcp_icl_ua);
	...
	case POWER_SUPPLY_TYPEC_SOURCE_DEFAULT:
		...
		case FLOAT_CHARGER_BIT:
			current_ua = chg->default_icl_ua;
                       smblib_dbg(chg, PR_MISC, "apsd_result: %d, default set icl to %d\n",
                               apsd_result->bit,  chg->default_icl_ua);
			break;
		default:
			current_ua = 0;
			break;
...
irqreturn_t default_irq_handler(int irq, void *data)
{
	...
	if (!strcmp(irq_data->name, "chgr-error") ||
		...
		!strcmp(irq_data->name, "aicl-done")) {
		power_supply_get_property(chg->usb_psy, POWER_SUPPLY_PROP_VOLTAGE_NOW, &pval);
		smblib_err(chg, "%s:vbus = %dmv, real_charger_type=%d\n",
			irq_data->name, pval.intval/1000, chg->real_charger_type);
	} else
#endif /*CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
...
irqreturn_t batt_temp_changed_irq_handler(int irq, void *data)
{
	...
	if (sgm_mag_charger_ok()) {
		...
		power_supply_set_property(chg->sgm_chg_psy, POWER_SUPPLY_PROP_HEALTH, &pval);
	}
#endif /*CONFIG_HISENSE_SGM41512_CHARGER*/
	...
	if (sgm_2btt_mag_charger_ok()) {
		if(sgm_use_battery1_ok()){		
			...
			power_supply_set_property(chg->sy_chg1_psy, POWER_SUPPLY_PROP_HEALTH, &pval);		
		}else{
			if (!chg->sy_chg2_psy) {
				chg->sy_chg2_psy= power_supply_get_by_name(SY6974X_CHG_BAT2_PSY_NAME);
				if (!chg->sy_chg2_psy) {
			...
			power_supply_set_property(chg->sy_chg2_psy, POWER_SUPPLY_PROP_HEALTH, &pval);
		}
	}
#endif /*CONFIG_HISENSE_SGM_DOUBLE_BATTERIES*/
	return IRQ_HANDLED;
...
#endif
#ifdef CONFIG_HISENSE_SGM_DOUBLE_BATTERIES
extern int sgm_2batt_typec_chg_enable(bool enable);
#endif /*CONFIG_HISENSE_SGM_DOUBLE_BATTERIES*/
void smblib_usb_plugin_locked(struct smb_charger *chg)
{
	...
	smblib_err(chg, "smblib_usb_plugin_locked: %d\n", vbus_rising);

/*Add for parallel charging check in factory mode*/
#if defined(CONFIG_SMB1355_SLAVE_CHARGER) && defined(CONFIG_HISENSE_CHARGE_FG_FUNCTION)
	smb1355_ftm_disabled = false;
#endif /*CONFIG_SMB1355_SLAVE_CHARGER && CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
	...
	if (vbus_rising) {
		...
		if (rc < 0)
			smblib_err(chg, "Couldn't to enable DPDM rc=%d\n", rc);
#ifdef CONFIG_HISENSE_SGM_DOUBLE_BATTERIES
	/*Enable typeC charging schedule*/
	 sgm_2batt_typec_chg_enable(true);
#endif /*CONFIG_HISENSE_SGM_DOUBLE_BATTERIES*/
		...
		/*Add for Factory mode, reduce parallel charger enable time*/
#ifdef CONFIG_HISENSE_CHARGE_FG_FUNCTION
		if (BOOT_FACTORY_MODE == dev_bi.bootmode)
...
#ifdef CONFIG_HISENSE_SGM_DOUBLE_BATTERIES
	/*Enable typeC charging schedule*/
	 sgm_2batt_typec_chg_enable(false);
#endif /*CONFIG_HISENSE_SGM_DOUBLE_BATTERIES*/
		if (chg->wa_flags & BOOST_BACK_WA) {
			data = chg->irq_info[SWITCHER_POWER_OK_IRQ].irq_data;
			if (data) {
		...
		smblib_update_usb_type(chg);
#ifdef CONFIG_FUSB_30X
		if(get_fusb_probe_status()) {
		    fusb_reinit_apsd_complete();
...
#ifdef CONFIG_TOUCHSCREEN_FT5X06_FH
extern int ft_ctp_work_with_ac_usb_plugin(int);
#endif
#ifdef CONFIG_TOUCHSCREEN_FTS_FH
extern int ft5526_ctp_work_with_ac_usb_plugin(int);
#endif
static void smb_ctp_work(struct work_struct *work)
{
#ifdef CONFIG_TOUCHSCREEN_FT5X06_FH
	ft_ctp_work_with_ac_usb_plugin((usb_flag ? 1 : 0));
#endif
#ifdef CONFIG_TOUCHSCREEN_FTS_FH
	ft5526_ctp_work_with_ac_usb_plugin((usb_flag ? 1 : 0));
#endif
}
#endif /* CONFIG_TOUCHSCREEN_USB_CHECK */
...
static void smblib_handle_apsd_done(struct smb_charger *chg, bool rising)
{
	...
	if(get_fusb_probe_status()) {
		...
		pr_err("%s: APSD complete singal done\n", __func__);
	}
#endif /*CONFIG_FUSB_30X*/
...
irqreturn_t usbin_ov_irq_handler(int irq, void *data)
{
	...
	if (stat & USBIN_OV_RT_STS_BIT) {
		...
		chg->usbin_ov = true;
		power_supply_changed(chg->usb_psy);
		smblib_err(chg, "%s:chg->usbin_ov =%d\n",
			irq_data->name,chg->usbin_ov);
#endif /*CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
	} else {
		cancel_delayed_work_sync(&chg->usbov_dbc_work);
		chg->dbc_usbov = false;
