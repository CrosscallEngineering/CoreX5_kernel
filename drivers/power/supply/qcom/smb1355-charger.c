...
bool smb1355_ftm_disabled = false;
					...
					union power_supply_propval *val)
{
	...
	mutex_unlock(&chip->suspend_lock);
	pr_buf_info("%s:smb1355 status is enabled/disabled(%d)\n", __func__, chip->charging_enabled);
	return rc;
}
...
#ifdef CONFIG_HISENSE_CHARGE_FG_FUNCTION
	/*If factory mode disable parallel, only allow factory to enable it*/
	pr_buf_info("%s:smb1355_ftm_disabled = %d\n", __func__, smb1355_ftm_disabled);
	if(smb1355_ftm_disabled) {
	    if(smb1355_ftm_disabled != disable)
	        return 0;
	}
#endif /*CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
...
#ifdef CONFIG_HISENSE_CHARGE_FG_FUNCTION
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		smb1355_ftm_disabled = !(bool)val->intval;
		rc = smb1355_set_parallel_charging(chip, smb1355_ftm_disabled);
		break;
#endif /*CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
	default:
