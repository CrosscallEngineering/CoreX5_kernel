...
#define pr_fmt(fmt)	"--PM-ALG: %s: " fmt, __func__
...
static void ttf_work(struct work_struct *work)
{
	...
	int batt_temp = 0, rbatt = 0, empty_volt_mv = 0,vbus = 0,charge_type = 0, msoc = 0;
...
#ifdef CONFIG_HISENSE_CHARGE_FG_FUNCTION
	ttf->ttf_ws = wakeup_source_register(NULL, "qcom-fg-alg");
#endif /*CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
	if (!ttf->iterm_delta)
