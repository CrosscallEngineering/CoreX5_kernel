...
#define pr_fmt(fmt) "--PM-SY6974-bat2: %s: " fmt, __func__
...
static int sy6974_charger_start_charge(struct sy6974_charger_info *info)
{
	...
	if (!info->chg_en_gpio) {
		...
		pr_buf_info("set CHG_EN 1, enable sy6974 for bat2\n");
	}
...
static void sy6974_charger_stop_charge(struct sy6974_charger_info *info)
{
	...
	if(!info->chg_en_gpio) {
		...
		pr_buf_err("###STOP charger### set chg_en_gpio to low, disable sy6974\n");
	}
		...
		const struct i2c_device_id *id)
{
	...
	pr_buf_info("sy6974_bat2 charger_probe success\n");
	return 0;
